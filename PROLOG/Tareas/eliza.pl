...
% pregunta lo que toma
template([te, gusta, tomar, s(_), '?'], [flagBeber], [3]).
...
% Lo que eliza toma: flagBeber
elizaBeber(X, R):- drink(X), R = ['Yes', i, like, X].
elizaBeber(X, R):- \+drink(X), R = ['No', i, dont, drink, X].
drink(agua).
drink(tea).
